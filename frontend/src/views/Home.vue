...
// Fetching all notebooks from the database and then setting the `notebooks` variable to the response.
onMounted(() => {
  loading.value = true;
  if (userID.value == "") {
    router.replace("/");
  ...
  axios
    ...
    .then(function (response: { data: { [x: string]: { id: string; title: string; pages: { id: string; title: string; content: string; }[]; }[]; }; }) {
...
// Creating a new notebook and then fetching all notebooks again.
...
// Deleting a notebook and then fetching all notebooks again.
async function deleteNotebook(notebookID: string) {
  ...
  await axios
    .get(import.meta.env.VITE_API_URL + "users/" + userID.value + "/notebooks")
    .then(function (response: { data: { [x: string]: { id: string; title: string; pages: { id: string; title: string; content: string; }[]; }[]; }; }): void {
      notebooks.value = response.data["notebooks"];
    });
  loading.value = false;
}

// A function that is called when the user clicks on a notebook. It sets the currentNotebookID to the
// notebook's ID and then redirects the user to the notebook page.
function gotoNotebook(notebook: Notebook) {
  currentNotebookID.value = notebook.id;
  router.push("/home/" + notebook.id);
}

// A function that is called when the user clicks the logout button. It sets the userID and userName to
// empty strings and then redirects the user to the login page.
function logout() {
  userID.value = "";
  userName.value = "";
  router.replace("/");
    ...
    <div class="m-5 grid grid-cols-4 gap-4 sm:grid-cols-2 lg:grid-cols-6">
      ...
      <div v-for="notebook in notebooks" :key="notebook.id">
        <div
          ...
          <h5 @click="gotoNotebook(notebook)" class="cursor-pointer mb-2 h-full text-2xl w-5/6 font-bold tracking-tight">
            {{ notebook.title }}
          </h5>
