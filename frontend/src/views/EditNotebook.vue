...
// Creating a new page with an empty id, title and content.
const currentPage = ref<Page>({
  id: "",
  content: "",
  title: "",
});
// Creating a new notebook with an empty id, title and pages.
...
// Creating a new page in the database and then getting the updated list of pages from the database.
...
// Deleting a page from the database and then getting the updated list of pages from the database.
...
// Updating the title of the notebook.
async function updateNotebookTitle() {
  // Setting the value of the `loading` variable to `true`.
  loading.value = true;
  await axios
    .put(
      import.meta.env.VITE_API_URL +
  ...
  // Setting the value of the `loading` variable to `false`.
  loading.value = false;
}
</script>
    ...
    <main
      ...
      <div id="sidebar" class="lg:w-1/6 md:w-1/4 h-full p-2 bg-slate-400 rounded-l-md">
        ...
        <div id="notebook_title" class="w-full h-20">
          <textarea name="" id="" rows="1" cols="200" wrap="off" v-model="notebook.title" @blur="updateNotebookTitle()"
        ...
        <div v-for="page in notebook.pages" class="mt-2">
          <button class="h-7 bg-gray-50 overflow-hidden dark:bg-gray-800 w-[calc((100%_-_1rem)*0.8)] rounded-md"
            @click="
              currentPage = page;
        ...
        <div class="" v-if="currentPage.id != '' && notebook.pages.length > 0">
          <div class="flex flex-row justify-between align-center">
            ...
            <button @click="updatePage()"
              ...
              <img src="https://img.icons8.com/ios-glyphs/30/null/save--v1.png" class="h-3 fill-inherit" />
            </button>
          </div>
          <textarea @blur="updatePage()" id="message" rows="1"
            class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            v-model="currentPage.title"></textarea>
          Page content:
          <div class="bg-white text-black">
            <QuillEditor ref="myEditor" theme="snow" v-model:content="currentPage.content" contentType="html" />
          </div>
        </div>
